...
enum { MACTION_0, MACTION_SUBMENU, MACTION_DATA };

enum { MITYPE_BIT, MITYPE_BYTE, MITYPE_SBYTE, MITYPE_WORD };
...
const uint8_t *     modes[]     = {String_NI, String_TI, String_SS, String_TC,
                          String_PW, String_BY, String_SM};
const uint8_t *strDMY[]  = {String_DMY1, String_MDY, String_DMY2, String_YMD};
const uint8_t  strADM[4] = {0x9C, 0x9F, 0xA8, 0x68}; // A D M a

const mbitdesc_t BitDesc = {0, 63, String_On, String_Off};

const mbitdesc_t InvBitDesc = {0, 63, String_Off, String_On};
...
__myevic__ void ProfileMenuIDraw(int it, int line, int sel) {
    ...
    dfParams_t *p =
        (dfParams_t *)(DATAFLASH_PROFILES_BASE + it * DATAFLASH_PARAMS_SIZE);

    DrawImage(4, line + 2, 0x0C + it);
    if (sel)
        InvertRect(0, line, 13, line + 12);
    ...
    case 2: // Plus
        ...
        case 5: // Protec
            if (++dfProtec > FIRE_PROTEC_MAX) {
                if (KeyTicks < 5)
                    dfProtec = FIRE_PROTEC_MIN;
                else
                    dfProtec = FIRE_PROTEC_MAX;
            }
            vret = 1;
            break;
        ...
        case 6: // Puffs off
            if (++dfPuffsOff > PUFFS_OFF_MAX) {
                if (KeyTicks < 5)
                    dfPuffsOff = PUFFS_OFF_MIN;
                else
                    dfPuffsOff = PUFFS_OFF_MAX;
            }
            vret = 1;
            break;
...
__myevic__ void IFMenuIDraw(int it, int line, int sel) {
    ...
    switch (it) {
    case 1: // 1Watt
        DrawStringRight(dfStatus.onewatt ? String_On : String_Off, 63,
                        line + 2);
        break;

    case 2: // 1C5F
        DrawStringRight(dfStatus.onedegree ? String_On : String_Off, 63,
                        line + 2);
        break;

    case 3: // Wake -+
        DrawStringRight(dfStatus.wakeonpm ? String_On : String_Off, 63,
                        line + 2);
        break;
    ...
    case 5: // TDom
        DrawStringRight(dfStatus.priopwr ? String_On : String_Off, 63,
                        line + 2);
        break;
...
__myevic__ void PreheatIDraw(int it, int line, int sel) {
    ...
    case 0:
        DrawFillRect(40, line, 63, line + 12, 0);
        DrawStringRight(dfStatus.preheat ? String_On : String_Off, 63,
                        line + 2);
        return;
...
__myevic__ void DrawPreheatDelay(int x, int y, int v, uint8_t dp, uint8_t z,
                                 uint8_t nd) {
    if (v == 0) {
...
__myevic__ int PreheatMEvent(int event) {
    ...
    if (event == EVENT_LONG_FIRE && CurrentMenuItem == 4) { // preheat delay off
        dfPHDelay              = 0;
        UpdateDFTimer          = 50;
        gFlags.refresh_display = 1;
        gFlags.edit_value      = 0;
...
__myevic__ void BVOMenuIDraw(int it, int line, int sel) {
    if (!it) {
        for (int i = 0; i < NumBatteries; ++i) {
            DrawValue(2 + 32 * (i & 1), 100 + (i >> 1) * 12, BattVolts[i], 2,
                      0x0B, 3);
...
__myevic__ void DrawCUS(int x, int y, int v, uint8_t dp, uint8_t z,
                        uint8_t nd) {
...
__myevic__ void MaxMenuIDraw(int it, int line, int sel) {
    ...
    case 4: // BY batts num
        DrawStringRight(dfStatus2.bybatts ? String_All : String_B1, 63,
                        line + 2);
        break;
    ...
    case 2: // +
        ...
        case 3: // Shunt Rez
            if (++AtoShuntRez > SHUNT_MAX_VALUE) {
                AtoShuntRez =
                    (KeyTicks == 0) ? SHUNT_MIN_VALUE : SHUNT_MAX_VALUE;
            }
            vret = 1;
            break;
    ...
    case 3: // -
        ...
        case 3: // Shunt Rez
            if (--AtoShuntRez < SHUNT_MIN_VALUE) {
                AtoShuntRez =
                    (KeyTicks == 0) ? SHUNT_MAX_VALUE : SHUNT_MIN_VALUE;
            }
            vret = 1;
            break;
...
__myevic__ void ScreenSaveOnEnter() { CurrentMenuItem = dfScreenSaver; }
...
__myevic__ void CoilsMenuIDraw(int it, int line, int sel) {
    ...
    switch (it) {
    case 3: // Cold
        t = dfStatus.IsCelsius ? dfColdLockTemp
                               : CelsiusToF((uint16_t)dfColdLockTemp);
    ...
    case 5: // Check
    {
        const uint8_t *s;
        DrawFillRect(36, line, 63, line + 12, 0);
        s = (dfStatus.chkmodeoff) ? String_Off
                                  : String_On; // String_No : String_Yes;
        DrawStringRight(s, 63, line + 2);
        break;
    }
    default:
...
__myevic__ void ScreenProtMenuIDraw(int it, int line, int sel) {
    ...
    case 4: // ModOff mode
        ...
        if (dfDimOffMode == 2) {
            ...
            DrawStringRight(dfDimOffMode ? String_OFF : String_Sleep, 63,
                            line + 2);
        }
...
__myevic__ void GameMEnter() { CurrentMenuItem = dfFBSpeed; }
...
__myevic__ void GameTtMEnter() { CurrentMenuItem = dfTTSpeed; }
...
__myevic__ void ModesIDraw(int it, int line, int sel) {
    ...
    (sel ? DrawImageInv + 1 : DrawImage + 1)(
        53, line + 2,
...
__myevic__ void CoilsSelectRez(uint8_t mode) {
    switch (mode) {
    case 0:
        CoilSelectedRez  = &dfRezNI;
        CoilSelectedLock = &dfRezLockedNI;
        break;
    case 1:
        CoilSelectedRez  = &dfRezTI;
        CoilSelectedLock = &dfRezLockedTI;
        break;
    case 2:
        CoilSelectedRez  = &dfRezSS;
        CoilSelectedLock = &dfRezLockedSS;
        break;
    case 3:
        CoilSelectedRez  = &dfRezTCR;
        CoilSelectedLock = &dfRezLockedTCR;
        break;
    default:
        break;
    }
...
__myevic__ void DrawTCRP(int x, int y, int v, uint8_t dp, uint8_t z,
                         uint8_t nd) {
    if (v == 0) {
...
__myevic__ void DrawLedColor(int x, int y, int v, uint8_t dp, uint8_t z,
                             uint8_t nd) {
    DrawValueRight(x, y + 2, v * 4, dp, z, nd);
    DrawImage(x + 1, y + 2, 0xC2);
}

__myevic__ void LedMenuEnter() { LEDGetColor(); }
...
__myevic__ int CurveMenuOnEvent(int event) {
    ...
    case 2: // Plus
        if (gFlags.edit_value) {
            // if ( CurrentMenuItem == 4 )
            //{
            if (dfCurveRepeatTimer < 50)
                dfCurveRepeatTimer += 1;
            else if (KeyTicks < 5)
    ...
    case 3: // Minus
        if (gFlags.edit_value) {
            // if ( CurrentMenuItem == 4 )
            //{
            if (dfCurveRepeatTimer > 0)
                dfCurveRepeatTimer -= 1;
            else if (KeyTicks < 5)
                dfCurveRepeatTimer = 50;
            //}
            // UpdateDFTimer = 50;
            // gFlags.refresh_display = 1;
            vret = 1;
...
const menu_t GameMenu = {String_Game,
                         &MiscsMenu,
                         GameMEnter + 1,
                         0,
                         GameISelect + 1,
...
const menu_t GameTtMenu = {String_Tetris,
                           &MiscsMenu,
                           GameTtMEnter + 1,
                           0,
                           GameTtISelect + 1,
                           ...
                           {{String_Easy, 0, 0, 0},
                            ...
                            {String_Back, 0, EVENT_PARENT_MENU, 0}}};
const menu_t ModesMenu = {String_Modes,
                          &VapingMenu,
                          ModesMEnter + 1,
                          ModesIDraw + 1,
                          0,
...
const mvaluedesc_t PreheatDelayDesc = {34,
                                       42,
                                       3,
                                       2,
                                       0,
...
const mdata_t PreheatDelayData = {&dfPHDelay, &PreheatDelayDesc, MITYPE_BYTE,
                                  0};

const mdata_t CurveDelayData = {&dfCUDelay, &PreheatDelayDesc, MITYPE_BYTE, 0};

const mdata_t PreheatSmartData = {&dfStatus2, &BitDesc, MITYPE_BIT, 2};

const menu_t PreheatMenu = {String_Preheat,
                            &VapingMenu,
                            0,
                            PreheatIDraw + 1,
                            0,
...
const mvaluedesc_t TCRNIDesc = {32,
                                59,
                                0,
                                0,
                                0,
...
const mvaluedesc_t TCRTIDesc = {32, 59, 0,    0, 0, 999, &DrawTCRP + 1,
                                0,  0,  0x0B, 1, 0, 366};

const mvaluedesc_t TCRSSDesc = {
    32, 59, 0, 0, 0, 999, &DrawTCRP + 1, 0, 0, 0x0B, 1, 0, 100 // 120
};

const mvaluedesc_t TCRMDesc = {32, 59, 0, 0, 1, 999, 0, 0, 0, 0x0B, 1, 0, 100};

const mdata_t TCRNIData = {&dfTCRP[0], &TCRNIDesc, MITYPE_WORD, 0};

const mdata_t TCRTIData = {&dfTCRP[1], &TCRTIDesc, MITYPE_WORD, 0};

const mdata_t TCRSSData = {&dfTCRP[2], &TCRSSDesc, MITYPE_WORD, 0};

const mdata_t TCRM1Data = {&dfTCRM[0], &TCRMDesc, MITYPE_WORD, 0};

const mdata_t TCRM2Data = {&dfTCRM[1], &TCRMDesc, MITYPE_WORD, 0};

const mdata_t TCRM3Data = {&dfTCRM[2], &TCRMDesc, MITYPE_WORD, 0};

const menu_t TCRSetMenu = {String_TCRSet,
                           &CoilsMenu,
                           0,
                           0,
                           0,
...
const menu_t CoilsMgmtMenu = {String_Manage,
                              &CoilsMenu,
                              CoilsMEnter + 1,
                              CoilsIDraw + 1,
                              CoilsISelect + 1,
...
const menu_t CoilsMenu = {
    ...
    {{String_Manage, &CoilsMgmtMenu, 0, MACTION_SUBMENU},
     ...
     {String_Profile, 0, 0,
      MACTION_SUBMENU}, // EVENT_PROFILE_MENU, MACTION_SUBMENU },
     {String_Cold, 0, 0, 0},
     {String_New, 0, 0, 0},
     {String_Check, 0, 0, 0},
     {String_Back, 0, EVENT_PARENT_MENU, 0}}};

const menu_t Object3DMenu = {String_3D,
                             &LogoMenu,
                             Object3DOnEnter + 1,
                             0,
                             0,
...
const mvaluedesc_t LedDesc = {34, 53,   0,    0, 0,  25, DrawLedColor + 1,
                              0,  0xC2, 0x0B, 1, 26, 26};

const mdata_t LedRedData = {&LEDRed, &LedDesc, MITYPE_BYTE, 0};

const mdata_t LedGreenData = {&LEDGreen, &LedDesc, MITYPE_BYTE, 0};

const mdata_t LedBlueData = {&LEDBlue, &LedDesc, MITYPE_BYTE, 0};

const menu_t LedMenu = {String_Led,
                        &MiscsMenu,
                        LedMenuEnter + 1,
                        0,
                        0,
...
const mdata_t FireFlip  = {&dfStatus, &BitDesc, MITYPE_BIT, 30};
const mdata_t SwapMP    = {&dfStatus2, &BitDesc, MITYPE_BIT, 3};
const mdata_t NBZC      = {&dfStatus, &BitDesc, MITYPE_BIT, 12};
const mdata_t Puff24    = {&dfStatus, &BitDesc, MITYPE_BIT, 19};
const menu_t  MiscsMenu = {String_Miscs,
                          &MainMenu,
                          0,
                          0,
                          0,
...
const mbitdesc_t HmsDesc = {36, 63, String_HM, String_hms};

const mdata_t ClkSizeData = {&dfStatus, &HmsDesc, MITYPE_BIT, 24};

const mdata_t OffModClock = {&dfStatus, &InvBitDesc, MITYPE_BIT, 29};
const mdata_t OnModClock  = {&dfStatus, &BitDesc, MITYPE_BIT, 4};
const menu_t  ClockMenu   = {
    String_Clock,
    &MainMenu,
    0,
...
const mdata_t DimOffDelayData = {&dfDimOffTimeout, &PreheatDelayDesc,
                                 MITYPE_BYTE, 0};
...
const mvaluedesc_t BVODesc = {32, 56, 2,    0, BVO_MIN, BVO_MAX, 0,
                              0,  0,  0x0B, 1, 127,     0};

const mdata_t BVO1Data = {&dfBVOffset[0], &BVODesc, MITYPE_SBYTE, 0};

const mdata_t BVO2Data = {&dfBVOffset[1], &BVODesc, MITYPE_SBYTE, 0};

const mdata_t BVO3Data = {&dfBVOffset[2], &BVODesc, MITYPE_SBYTE, 0};

const mdata_t BVO4Data = {&dfBVOffset[3], &BVODesc, MITYPE_SBYTE, 0};

const menu_t BVOMenu = {String_BVO,
                        &ExpertMenu,
                        0,
                        BVOMenuIDraw + 1,
                        0,
...
const mvaluedesc_t CUSVoltDesc = {30, 40, 3,    2, 270, 423, &DrawCUS + 1,
                                  0,  0,  0x0B, 1, 424, 0};
const mvaluedesc_t CUSPercDesc = {30, 40, 2,    0, 1,   99, &DrawCUS + 1,
                                  0,  0,  0x0B, 1, 100, 0};
const mdata_t CUS1Voltage = {&CustomBattery.V2P[0].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS2Voltage = {&CustomBattery.V2P[1].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS3Voltage = {&CustomBattery.V2P[2].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS4Voltage = {&CustomBattery.V2P[3].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS5Voltage = {&CustomBattery.V2P[4].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS6Voltage = {&CustomBattery.V2P[5].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS7Voltage = {&CustomBattery.V2P[6].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS8Voltage = {&CustomBattery.V2P[7].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS9Voltage = {&CustomBattery.V2P[8].voltage, &CUSVoltDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS10Voltage = {&CustomBattery.V2P[9].voltage, &CUSVoltDesc,
                              MITYPE_WORD, 0};
const mdata_t CUS11Voltage = {&CustomBattery.V2P[10].voltage, &CUSVoltDesc,
                              MITYPE_WORD, 0};

//
const mdata_t CUS2Percent = {&CustomBattery.V2P[1].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS3Percent = {&CustomBattery.V2P[2].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS4Percent = {&CustomBattery.V2P[3].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS5Percent = {&CustomBattery.V2P[4].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS6Percent = {&CustomBattery.V2P[5].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS7Percent = {&CustomBattery.V2P[6].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS8Percent = {&CustomBattery.V2P[7].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS9Percent = {&CustomBattery.V2P[8].percent, &CUSPercDesc,
                             MITYPE_WORD, 0};
const mdata_t CUS10Percent = {&CustomBattery.V2P[9].percent, &CUSPercDesc,
                              MITYPE_WORD, 0};

const menu_t CUSMenu = {String_CUS,
                        &ExpertMenu,
                        0,
                        CUSMenuIDraw + 1,
                        0,
...
const menu_t MAXMenu = {String_MAX_s,
                        &ExpertMenu,
                        0,
                        MaxMenuIDraw + 1,
                        0,
...
const menu_t ExpertMenu = {String_Expert,
                           &MainMenu,
                           0,
                           ExpertMenuIDraw + 1,
                           0,
...
const menu_t ScreenSaveMenu = {String_Saver,
                               &ScreenMenu,
                               ScreenSaveOnEnter + 1,
                               0,
...
const mbitdesc_t TopBotDesc = {0, 0, String_Mid, String_Top};

const mdata_t LogoShowData = {&dfStatus, &InvBitDesc, MITYPE_BIT, 3};

const mdata_t      LogoWhereData = {&dfStatus, &TopBotDesc, MITYPE_BIT, 23};
const mvaluedesc_t HideLogoDesc  = {40, 55,   0,    0, 0,  15, 0,
                                   0,  0x94, 0x0B, 1, 16, 3};
const mdata_t HideLogoData = {&dfHideLogo, &HideLogoDesc, MITYPE_BYTE, 0};
const menu_t  LogoMenu     = {String_Logo,
                         &ScreenMenu,
                         0,
                         0,
                         0,
...
const mdata_t ScreenInvData = {&dfStatus, &BitDesc, MITYPE_BIT, 18};
...
const menu_t ClicksMenu = {String_Clicks,
                           &IFMenu,
                           0,
                           ClicksMenuIDraw + 1,
                           0,
...
const menu_t IFMenu = {String_Interface,
                       &MainMenu,
                       0,
                       IFMenuIDraw + 1,
                       0,
...
const mvaluedesc_t BoostDesc = {34, 52,   0,    0, 0,   100, 0,
                                0,  0xC2, 0x0B, 1, 101, 50};

const mdata_t BoostData = {&dfTCBoost, &BoostDesc, MITYPE_WORD, 0};

const mvaluedesc_t PIDPDesc = {24, 63, 0,    0, PID_P_MIN,     PID_P_MAX, 0,
                               0,  0,  0x0B, 1, PID_P_MAX + 1, PID_P_DEF};

const mdata_t PIDPData = {&dfPID.P, &PIDPDesc, MITYPE_WORD, 0};

const mvaluedesc_t PIDIDesc = {24, 63, 0,    0, PID_I_MIN,     PID_I_MAX, 0,
                               0,  0,  0x0B, 1, PID_I_MAX + 1, PID_I_DEF};

const mdata_t PIDIData = {&dfPID.I, &PIDIDesc, MITYPE_WORD, 0};

const mvaluedesc_t PIDDDesc = {24, 63, 0,    0, PID_D_MIN,     PID_D_MAX, 0,
                               0,  0,  0x0B, 1, PID_D_MAX + 1, PID_D_DEF};

const mdata_t PIDDData = {&dfPID.D, &PIDDDesc, MITYPE_WORD, 0};

const menu_t AlgoMenu = {String_Algo,
                         &VapingMenu,
                         0,
                         AlgoMenuIDraw + 1,
                         0,
...
const mdata_t CurveEnaData = {&dfStatus, &CurveEnaDesc, MITYPE_BIT, 28};
...
const mdata_t VVLite     = {&dfStatus, &BitDesc, MITYPE_BIT, 31};
const mdata_t APuffTime  = {&dfStatus, &BitDesc, MITYPE_BIT, 6};
const menu_t  VapingMenu = {
    String_Vaping,
    &MainMenu,
    0,
...
const menu_t ProfileMenu = {String_Profile,
                            0,
                            0,
                            ProfileMenuIDraw + 1,
                            0,
...
const menu_t MainMenu = {String_Menus,
                         0,
                         0,
                         0,
                         0,
...
__myevic__ void DrawMenuData(int line, int sel, const mdata_t *data) {
    ...
    case MITYPE_BIT: {
        ...
        if (!desc) {
            ...
            DrawStringRight(b ? desc->on : desc->off, desc->pos ?: 63,
                            line + 2);
        }
        break;
    }
    ...
    case MITYPE_WORD: {
        ...
        if (desc->draw) {
            ...
            DrawValueRight(desc->posr, line + 2, v, desc->dp, desc->z,
                           desc->nd);
...
__myevic__ void DrawMenu() {
    ...
    if (CurrentMenuItem >= CurrentMenu->nitems)
        CurrentMenuItem = 0;
    ...
    for (int i = 0; i < 8; ++i) {
        if (firstitem + i >= CurrentMenu->nitems)
            break;
        ...
        if (CurrentMenu->on_drawitem) {
            CurrentMenu->on_drawitem(firstitem + i, 18 + 14 * i,
                                     CurrentMenuItem == firstitem + i);
        }
    }
}
...
__myevic__ int MenuEvent(int event) {
    ...
    case 1: // fire
    {
        ...
        if (CurrentMenu->on_clickitem)
            CurrentMenu->on_clickitem();
        ...
        if (mi->action) {
            ...
            case MACTION_SUBMENU: {
                ...
                if (CurrentMenu->on_enter)
                    CurrentMenu->on_enter();
    ...
    case 2: // plus
    {
        ...
        if (CurrentMenu->on_selectitem)
            CurrentMenu->on_selectitem();
