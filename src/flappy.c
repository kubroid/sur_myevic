...
__myevic__ void fbCallTimeouts() {
    ...
    for (fbCurrentTimeout = 0; fbCurrentTimeout < FB_NUM_TIMERS;
         ++fbCurrentTimeout) {
...
//=========================================================================
//----- (00001348) --------------------------------------------------------
__myevic__ void fbCLSBuf() { MemClear2(0, ScreenBuffer, SCREEN_BUFFER_SIZE); }
...
//=========================================================================
//----- (0000805E) --------------------------------------------------------
__myevic__ void fbDrawSprite(int x, int y, int w, int h,
...
//=========================================================================
//----- (000080BA) --------------------------------------------------------
__myevic__ void fbDrawRect(int x1, int y1, int x2, int y2, int color,
                           int fill) {
    int dx, dy;
    int i, j, t;
...
__myevic__ void fbGame() {
    ...
    if (fbColumn1.x == 21 || fbColumn2.x == 21 || fbColumn3.x == 21)
        ++fbScore;

    if (fbBirdLine >= 48)
        fbDead = 1;
    else
        fbBirdAnim(fbBirdLine);

    if (fbColumn1.x >= -8 && fbColumn1.x <= 31 &&
        (fbColumn1.top_bot + 6 > fbBirdLine ||
         fbColumn1.bot_top - 12 < fbBirdLine)) {
        fbDead = 1;
    }

    if (fbColumn2.x >= -8 && fbColumn2.x <= 31 &&
        (fbColumn2.top_bot + 6 > fbBirdLine ||
         fbColumn2.bot_top - 12 < fbBirdLine)) {
        fbDead = 1;
    }

    if (fbColumn3.x >= -8 && fbColumn3.x <= 31 &&
        (fbColumn3.top_bot + 6 > fbBirdLine ||
         fbColumn3.bot_top - 12 < fbBirdLine)) {
        fbDead = 1;
    }
...
//=========================================================================
//----- (00006758) --------------------------------------------------------
__myevic__ void fbSetBirdColumn(uint8_t c) { fbBirdColumn = c; }
