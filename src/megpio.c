...
__myevic__ void GPD_IRQHandler() {
    if (GPIO_GET_INT_FLAG(PD, GPIO_PIN_PIN7_Msk)) {
        ...
        if (gFlags.usb_attached && (NumBatteries == 1) &&
            (BatteryVoltage >= 414)) // 414
        {
            BattProbeCount = 1;
        ...
        if (ISPRESA75W || ISVTCDUAL || ISCUBOID || ISCUBO200 || ISRX200S ||
            ISRX23 || ISRX300 || ISPRIMO1 || ISPRIMO2 || ISPREDATOR) {
...
//----- (00002342) --------------------------------------------------------
__myevic__ void GPF_IRQHandler() { PF->INTSRC = PF->INTSRC; }
...
__myevic__ void InitGPIO() {
    ...
    if (ISCUBOID || ISCUBO200 || ISRX200S || ISRX23) {
        ...
        if (ISRX300 || ISRX2 || ISSINFJ200 || ISRX217) {
            SYS->GPF_MFPL &=
                ~(SYS_GPF_MFPL_PF5MFP_Msk | SYS_GPF_MFPL_PF6MFP_Msk);
            SYS->GPF_MFPL |=
                SYS_GPF_MFPL_PF5MFP_GPIO | SYS_GPF_MFPL_PF6MFP_GPIO;
            PF5 = 0;
    ...
    PE10 = 0; // 0x40004928
    GPIO_SetMode(
        PE, GPIO_PIN_PIN10_Msk,
        GPIO_MODE_OUTPUT); // 0x40004100 (0x400 = 0x40004928 ASR 0x14) 1
    ...
    GPIO_SetMode(
        PE, GPIO_PIN_PIN12_Msk,
        GPIO_MODE_OUTPUT); // 0x40004100 (0x1000 = 0x40004930 ASR 0x12) 1
    ...
    if (ISEGRIPII || ISEVICAIO) {
        ...
        PB5 = 0; // Green
        GPIO_SetMode(PB,
                     GPIO_PIN_PIN3_Msk | GPIO_PIN_PIN4_Msk | GPIO_PIN_PIN5_Msk,
                     GPIO_MODE_OUTPUT);
    }
    ...
    if (ISCUBO200 || ISRX200S || ISRX23 || ISRX300 || ISGEN3) {
        ...
        PD1 = 0; // 0x400048C4
        GPIO_SetMode(PD, GPIO_PIN_PIN0_Msk | GPIO_PIN_PIN1_Msk,
                     GPIO_MODE_OUTPUT); // 0x400040C0 3 1
    } else if (ISRX2 || ISRX217) {
        PA3 = 1;
        GPIO_SetMode(PA, GPIO_PIN_PIN3_Msk, GPIO_MODE_OUTPUT); // pa 8 1
    } else if (ISSINFJ200) {
    ...
    GPIO_SET_DEBOUNCE_TIME(GPIO_DBCTL_DBCLKSRC_LIRC,
                           GPIO_DBCTL_DBCLKSEL_1024); // 0x3A 0x40004440
}
