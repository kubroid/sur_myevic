...
const uint8_t TempCoefsNI[] = {10,  16,  33,  48,  66,  82,  98,
                               120, 133, 150, 160, 180, 190, 210,
                               220, 240, 250, 255, 255, 255, 255};

// const uint8_t TempCoefsTI[] =
//		{	53, 53, 53, 53, 53, 53, 53, 53, 53, 53, 54,
//			55, 55, 55, 55, 56, 56, 56, 56, 56, 56		};

const uint8_t TempCoefsTI[] = {10,  10,  20,  30,  41,  51,  61,
                               71,  81,  91,  103, 114, 124, 134,
                               144, 155, 165, 175, 185, 195, 205};
...
__myevic__ void InitPWM() {
     ...
     * @param[in] u32CMR Comparator of specified channel. Valid values are
    between 0~0xFFFF
     * @return None
    ...
    ((pwm)->CMPDAT[(u32ChannelNum)]= (u32CMR))
    **/
...
__myevic__ uint16_t GetAtoVWVolts(uint16_t pwr, uint16_t r) {
    ...
    if (AtoError || !r) // AtoRez
    {
        ...
        if (volts < AtoMinVolts)
            volts = AtoMinVolts;
        if (volts > AtoMaxVolts)
            volts = AtoMaxVolts;
    }
...
//=========================================================================
//----- (00001334) --------------------------------------------------------
__myevic__ uint16_t CelsiusToF(uint16_t tc) { return (tc * 9) / 5 + 32; }

//=========================================================================
//----- (000022A8) --------------------------------------------------------
__myevic__ uint16_t FarenheitToC(uint16_t tf) { return (5 * (tf - 32)) / 9; }
...
__myevic__ void StopFire() {
    ...
    if (ISVTCDUAL) {
        ...
        GPIO_SetMode(PD, GPIO_PIN_PIN7_Msk,
                     GPIO_MODE_INPUT); // 0x400040C0 0x80 0
    ...
    if (gFlags.firing) {
        ...
        if (FireDuration > 5) {
            ...
            if (dfTimeCount > 999999)
                dfTimeCount = 0;
            if (++dfPuffCount > 99999)
                dfPuffCount = 0;
            UpdatePTTimer   = 80;
...
__myevic__ void ReadAtoCurrent() {
    ...
    if (gFlags.firing || gFlags.probing_ato) {
        ...
        if (gFlags.firing
            ...
            &&
            160 * 13 * adcAtoVolts / 100 * AtoShuntRez <
                30 * (adcShunt1 + adcShunt2) * arez &&
            AtoCurrent > 50       // 5.0A
        ...
        //	Definitely useless.
        //	else if ( ( current1 >= 255 || current2 >= 255 ) &&
        //! ISMODEBY(dfMode) )
        //	{
        //		// Original comparisons where "> 256".
        //		// This case can only occur if shunt resistance value is
        // below 1 mOhm,
        //		// due to the resolution of the EADC. On all hardware
        // versions, shunts
        //		// are between 1.00 and 1.25 mOhm so we could never get
        // there.
        //		// This test seems useless anyway since AtoPowerLimit()
        // is
        // called
        ...
        /*
                        myprintf(	"\n"
                                                " Short %d!
           u32ADValue_Res_temp(%d) u32ADValue_CurVol_temp(%d)"
                                                " g_u16DetRes_I(%d.%d)
           u16Res(%d) g_u32Set_OutVol(%d).\n",
                                                s,
                                                adcShunt1,
                                                adcAtoVolts,
                                                AtoCurrent / 10,
...
__myevic__ void ReadAtoTemp() {
    ...
    if (AtoRezMilli && AtoRezMilli <= 3000 && dfResistance <= 150 &&
        ...
        if (AtoRezMilli <= base_rez) {
            ...
            AtoTemp = 100 * (AtoRezMilli - base_rez) / TCR +
                      CelsiusToF(dfColdLockTemp); // 68 Ni +140
        }
        // else if ( dfTempAlgo == 2 )
        //{
        //        //new table need for TCR calc
        //        AtoTemp = 100 * ( AtoRezMilli - base_rez ) / TCR + CelsiusToF(
        //        dfColdLockTemp );
...
__myevic__ void ReadAtomizer() {
    ...
    if (TargetVolts) {
        ...
        if (gFlags.firing) {
            uint32_t pwr =
                AtoCurrent * AtoCurrent * AtoRezMilli / 10000; // P = I^2 * R
            MilliJoules += pwr;                                //*10
            MilliJoulesDay += pwr;
        }

        // myprintf( "%d(%d,%d,%d)\n", AtoRezMilli, ADCAtoSum, ADCShuntSum1,
        // ADCShuntSum2 );
        ...
        if (AtoRezMilli >= 5) {
            ...
            if ((AtoStatus == 4 || !AtoStatus || AtoStatus == 3) &&
                AtoRezMilli >= 50 && AtoRezMilli <= 3500 &&
...
__myevic__ void AtoWarmUp() {
    ...
    do {
        ...
        if (gFlags.firing || !gFlags.pwm_pll || !WUC ||
            (WUC - WarmUpCounter >= 20)) {
            WUC = WarmUpCounter;
            RegulateBuckBoost();
        }
...
__myevic__ void TweakTargetVoltsVW() {
    ...
    if (dfStatus.pcurve) {
        ...
        if (dfCurveRepeatTimer &&
            CurveRepeatTimerDuration > dfCurveRepeatTimer) // 10 = 1sec
            CurveRepeatTimerDuration = 0;

        for (t = 0; t < PWR_CURVE_PTS - 1; ++t)
            // if (( dfPwrCurve[t+1].time > FireDuration ) || (
            // dfPwrCurve[t+1].time == 0 ))
            if ((dfPwrCurve[t + 1].time > CurveRepeatTimerDuration) ||
                (dfPwrCurve[t + 1].time == 0))
                break;
...
const uint16_t SMARTRezValues[] = {50, 60, 100, 150};

// SMART mode power ranges
const uint16_t SMARTPowers[] = {250, 300, 200, 280, 180,
                                250, 160, 200, 200, 750};
...
__myevic__ void ProbeAtomizer() {
    if ((ISVTCDUAL && (BatteryStatus == 2 || !PA3)) ||
        ((ISCUBOID || ISCUBO200 || ISRX200S || ISRX23 || ISRX300 || ISGEN3) &&
         (BatteryStatus == 2 || !PF0)) ||
        ((ISPRIMO1 || ISPRIMO2 || ISPREDATOR || ISINVOKE) &&
         (BatteryStatus == 2 || !PD1)) ||
    ...
    if (AtoStatus == 4) {
        ...
        if (!ISMODETC(dfMode)) {
            if (!dfStatus2.vwrezlock && dfResistance > AtoRez)
                dfResistance =
                    AtoRez; // real goes down to cold (may be in TC too?)
        }
    } else {
        AtoRez    = 0;
        AtoMillis = 0;
    ...
    */
    if (AtoError == LastAtoError && (AtoRez > LastAtoRez) &&
        (AtoRez <= (LastAtoRez + 3))) {
        AtoRez    = LastAtoRez;
        AtoMillis = LastAtoMillis;
    }
    ...
    if (AtoRez != LastAtoRez || AtoError != LastAtoError) {
        ...
        if (Screen != 2 || gFlags.firing)
            gFlags.refresh_display = 1; // no refresh while scr2 delay
...
__myevic__ void SwitchRezLock(int lock) {
    ...
    case 5: // bypass
        ...
        /*
                                ...
                                dfVWVolts = GetAtoVWVolts( dfPower, dfResistance
           ); //AtoRez
                                dfVVLockedVolt = dfVWVolts;
                                RezMillis = AtoMillis;
        */
...
const uint32_t BoardTempTable[] = {
    34800, 26670, 20620, 16070, 12630, 10000, 7976, 6407, 5182, 4218, 3455,
    2847,  2360,  1967,  1648,  1388,  1175,  999,  853,  732,  630}; // 21
    ...
    if (ISRX300) {
        ...
        if (sample != 4096 && sample > 100) // 4095
        {
            ...
            if (tdr <= 630) {
                ...
                v7 = 0;
                do {
                    if (v6 * v7 + 10 * v4 <= 10 * tdr &&
                        (v7 + 1) * v6 + 10 * v4 > 10 * tdr)
                        break;
                    ++v7;
                } while (v7 < 5);
...
const uint32_t AkkuTempTable[] = {
    641800, 494500, 383000, 299400, 236000, 187500, 150000, 120500, 97460,
    79250,  64800,  53270,  44070,  36640,  30610,  25700,  21650}; // 17
...
int condTrue() { return 1; }

int condFalse() { return 0; }

// Simulate Joyetech with segments

int condCross1() { return (AlgoCtl.atemp > AlgoCtl.ttemp); }

int condCross2() {
    return ((AlgoCtl.atemp < AlgoCtl.ttemp) && (AlgoCtl.stage->counter > 100));
}

int condFinal() { return 0; }
...
int condBoostEnd() {
    return (AlgoCtl.atemp > AlgoCtl.ttemp * AlgoCtl.boost /
                                100); //|| gFlags.decrease_voltage );
}

int condNearCross1() { return (AlgoCtl.atemp > AlgoCtl.ttemp - 60); }
...
__myevic__ void TweakTargetVoltsPID() {
    ...
    pwr = AlgoCtl.error * dfPID.P + AlgoCtl.integ * dfPID.I / 50 +
          ediff * dfPID.D * 50;
