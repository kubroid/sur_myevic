...
#define DATAFLASH_PROFILES_BASE                                                \
    (DATAFLASH_PROFILES_END - DATAFLASH_PROFILES_SPACE)
...
typedef struct {
    ...
    /* 00001000 */ unsigned int
                                nbrc : 1; // lsls 0x13        new battery reset counters
...
typedef struct {
    ...
    /* 00000020 */ unsigned int
                                splash0 : 1; // 0-00 off; 1-01 on; 2-10 on with box name //11
    /* 00000040 */ unsigned int splash1 : 1;
    /* 00000080 */ unsigned int bybatts : 1; // batteries voltage for bypass
                                             // mode, 0 - 1batt, 1 - all batts
...
typedef struct dfParams {
    ...
    /* 004E */ // uint8_t		Unused4E[38];         //recheck for
               // PWR_CURVE_PTS value /* 004E */ = 10 pts /* 0062 */ = 20 pts
    /* 0062 */ uint8_t Unused4E[6]; // recheck for PWR_CURVE_PTS value /* 004E
                                    // */ = 10 pts /* 0062 */ = 20 pts
    ...
    /* 0074 */ uint8_t
                          CurveRepeatTimer; // 0.1s = 1 1s = 10...    was TempCoefsTi
    /* 0075 */ int8_t     AkkuTempCorr;
    /* 0076 */ uint16_t   LEDColor; //	former 2-bytes pad
    /* 0078 */ dfStatus_t Status;
    /* 007C */ uint8_t    BattLine; // 0- %+V,1- %+small,2-v+small,3-all_small
                                 // //was uint16_t	AtoRez;
    ...
    uint8_t
            DimOffTimeout; //  FE      0-255 0-4:15  ( x \ 60 & ":" & (x Mod 60) )
...
#define DATAFLASH_FREE_SIZE                                                    \
    (FMC_FLASH_PAGE_SIZE - DATAFLASH_PARAMS_SIZE - DATAFLASH_INFOS_SIZE -      \
     DATAFLASH_PARAMS2_SIZE)
