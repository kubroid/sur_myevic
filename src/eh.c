...
__myevic__ void EventHandler() {
    ...
    case 1: // Fire
    {
        if (dfStatus.off) {
            ...
            } else if (!gFlags.battery_charging && !dfStatus.offmodclock &&
                       Screen == 0) {
                SetScreen(60, 10);
            }
            return;
        ...
        if ((BoardTemp > dfMaxBoardTemp) || (ISSINFJ200 && (AkkuTemp > 70))) {
            Overtemp();
            return;
        }
        ...
        if (gFlags.new_rez_ni && dfMode == 0) {
            ...
            if (!dfRezNI) {
                ...
                if (3 * dfRezNI >= NewRez) {
                    if (dfRezNI + dfRezNI * dfNewRezPerc / 100 <
                            NewRez // dfRezNI / 20
                        && dfRezNI + 1 < NewRez
                        //&&	!dfRezLockedNI
                        ) {
                    ...
                    if (((dfRezNI - dfRezNI * dfNewRezPerc / 100 <= NewRez ||
                          dfRezNI - 1 <= NewRez) // dfRezNI / 20
                         && (dfRezNI + dfRezNI * dfNewRezPerc / 100 >= NewRez ||
                             dfRezNI + 1 >= NewRez)) // dfRezNI / 20
                        //||
                        //	(	dfRezLockedNI
                        //	&&	(dfRezNI - dfRezNI / 10 <= NewRez ||
                        // dfRezNI - 4 <= NewRez))
                        ) {
        ...
        if (gFlags.new_rez_ti && dfMode == 1) {
            ...
            if (!dfRezTI) {
                ...
                if (2 * dfRezTI >= NewRez) {
                    ...
                    if (((dfRezTI - dfRezTI * dfNewRezPerc / 100 <= NewRez ||
                          dfRezTI - 1 <= NewRez) &&
                         (dfRezTI + dfRezTI * dfNewRezPerc / 100 >= NewRez ||
                          dfRezTI + 1 >= NewRez))
                        //||
                        //	(	dfRezLockedTI
                        //	&&	(dfRezTI - dfRezTI / 10 <= NewRez ||
                        // dfRezTI - 4 <= NewRez))
                        ) {
            ...
            if (!dfRezSS) {
                ...
                if (3 * dfRezSS >= 2 * NewRez) {
                    if (dfRezSS + dfRezSS * dfNewRezPerc / 100 < NewRez &&
                        ...
                        if (((dfRezSS - dfRezSS * dfNewRezPerc / 100 <=
                                  NewRez ||
                              dfRezSS - 1 <= NewRez) &&
                             (dfRezSS + dfRezSS * dfNewRezPerc / 100 >=
                                  NewRez ||
                              dfRezSS + 1 >= NewRez))
                            //||
                            //	(	  dfRezLockedSS
                            //	&&	( dfRezSS - dfRezSS / 10 <= NewRez ||
                            // dfRezSS - 4 <= NewRez ) )
                            ) {
                            dfResistance = dfRezSS;
                            RezMillis    = (dfMillis & 0xf00) >> 8;
                        } else if (dfRezSS - dfRezSS * dfNewRezPerc / 100 >
                                       NewRez &&
                                   dfRezSS - 1 > NewRez) {
                            dfResistance = NewRez;
                            RezMillis    = NewMillis;
            ...
            if (!dfRezTCR) {
                ...
                if (3 * dfRezTCR >= 2 * NewRez) {
                    if (dfRezTCR + dfRezTCR * dfNewRezPerc / 100 < NewRez &&
                        ...
                        if (((dfRezTCR - dfRezTCR * dfNewRezPerc / 100 <=
                                  NewRez ||
                              dfRezTCR - 1 <= NewRez) &&
                             (dfRezTCR + dfRezTCR * dfNewRezPerc / 100 >=
                                  NewRez ||
                              dfRezTCR + 1 >= NewRez))
                            //||
                            //	(	  dfRezLockedTCR
                            //	&&	( dfRezTCR - dfRezTCR / 10 <= NewRez ||
                            // dfRezTCR - 4 <= NewRez ) )
                            ) {
                            dfResistance = dfRezTCR;
                            RezMillis    = (dfMillis & 0xf000) >> 12;
                        } else if (dfRezTCR - dfRezTCR * dfNewRezPerc / 100 >
                                       NewRez &&
                                   dfRezTCR - 1 > NewRez) {
                            dfResistance = NewRez;
                            RezMillis    = NewMillis;
        ...
        if (ISMODEVW(dfMode)) {
            ...
            LowBatVolts =
                (BatteryVoltage > BatteryCutOff + 100) ? 0 : BatteryVoltage;
        }
    ...
    */
    ...
    case 3: // - (minus or left) button
    {
        ...
        if (Screen == 5) //&& dfStealthOn != 1 ) //charge scr on off
        {
            ...
            gFlags.screen_on ^= 1;
            ScreenDuration =
                ScrChargeTimes[dfScrChargeTime]; // ScreenDuration = 0;
            gFlags.refresh_display = 1;
        }
        ...
        } else if (Screen == 51) // new coil screen [OLD]
        {
            ...
            if (EditModeTimer) // -
            {
                ...
                if (gFlags.edit_capture_evt) {
                    if (EditItemIndex == 2) {
                        dfStatus.priopwr
                            ? TempMinus()
                            : PowerMinus(&dfTCPower, 10, MaxTCPower);
            ...
            {
                if (ISMODETC(dfMode)) {
                    dfStatus.priopwr ? PowerMinus(&dfTCPower, 10, MaxTCPower)
                                     : TempMinus();
    ...
    case 2: // + (plus or right) button
    {
        ...
        } else if (Screen == 1) // main screen
        {
            ...
            if (EditModeTimer) {
                ...
                case 2:
                    dfStatus.priopwr ? TempPlus()
                                     : PowerPlus(&dfTCPower, 10, MaxTCPower);
                    gFlags.edit_capture_evt =
                        1; // for use minus button here too
                    break;
            ...
            } else // plus button, not in edit
            {
                ...
                case 2:
                case 3:
                    dfStatus.priopwr ? PowerPlus(&dfTCPower, 10, MaxTCPower)
                                     : TempPlus();
                    break;
