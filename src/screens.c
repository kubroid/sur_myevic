...
const uint8_t ScrSaveTimes[8] = {
    5, 15, 30, 1, 5, 15, 30, 0}; //, 255 }; //{ 1, 2, 5, 10, 15, 20, 30, 0 };
...
__myevic__ void DrawScreen() {
    ...
    if ((!PE0 || AutoPuffTimer) && Screen == 2 && FireDuration &&
        FireDuration != CurrentFD) {
    ...
    if (gFlags.refresh_display) {
        ...
        case 60: // Screen Saver
            ...
            if (dfStatus.off) {
                DisplaySetContrast(
                    dfContrast); // main contrast in off state fire clock
                gFlags.MainContrast = 1;
            } else {
                if (gFlags.MainContrast) {
                    DisplaySetContrast(dfContrast2);
    ...
    // FireClicksEvent  FireClickCount CurrentFD ScreenDuration PreheatDelay
    // SleepTimer KeyUpTimer
    ...
    case 0: // Black
        if (dfStatus.off) {
            ...
            if ((!gFlags.firing) && (dfStealthOn != 1) && (SleepTimer > 0) &&
                (dfScreenSaver > 0) && (GetScreenProtection() > 0) &&
                (scrSaveOnce)
...
__myevic__ void ShowMenus() { DrawMenu(); }
    ...
    if (gFlags.battery_10pc && !gFlags.battery_charging) {
        ...
        if (dfBattLine == 1 || dfBattLine == 2) {
            ...
            if (gFlags.batteries_ooe && gFlags.draw_battery) {
                ...
                DrawFillRectLines(32, 119, (25 * BatteryTenth / 10 + 31), 123,
                                  1);
            }
        } else if (dfBattLine == 0) {
            ...
            //	else if ( BatteryTenth )
            //	{
            //		DrawFillRectLines( 10, 119, (4 * BatteryTenth + 9), 123,
            // 1
            //);
            //	}
        } else if (dfBattLine == 3) {
                ...
                for (i = 0; i < NumBatteries; ++i) {
                    ...
                    if (BatteryTenthAll[i]) {
                        if (NumBatteries == 2)
                            DrawFillRectLines(
                                x0 + 1, y0 + 1,
                                (25 * BatteryTenthAll[i] / 10 + x0), 123, 1);
                        else
                            DrawFillRect(
                                x0, y0, (25 * BatteryTenthAll[i] / 10 + x0 + 2),
                                y, 1);
                    }
                }
            }
...
__myevic__ void ShowBatCharging() {
    ...
    */
    ...
    /*
            if (( dfScreenSaver == SSAVER_CLOCK ) || ( dfScreenSaver ==
       SSAVER_LOGO ))
            {
...
__myevic__ void ShowSetDate() {
    const uint8_t cols[4][3] = {{0x1E, 0x1B, 0x0F},
                                {0x1E, 0x0F, 0x1B},
                                {0x1E, 0x1B, 0x0F},
                                {0x0F, 0x1B, 0x1E}};
...
//=========================================================================
__myevic__ int IsMenuScreen() { return ((Screen >= 101) && (Screen <= 107)); }
...
__myevic__ void ShowPowerCurve() {
    ...
    for (int i = 0; i < PWR_CURVE_PTS; ++i) {
        ...
        DrawVLine(7 + dfPwrCurve[i].power / 4, 20 + 2 * t1, 19 + 2 * t2, 1);
