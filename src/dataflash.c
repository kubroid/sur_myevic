...
#define __PIDATTR__                                                            \
    __attribute__((aligned(4))) __attribute__((section(".productid")))
...
__myevic__ void InitSetPowerVoltMax() {
    ...
    } else if (ISPRIMOSE || ISPRIMOMINI || ISVTWO || ISEGRIPII || ISCUBOMINI ||
               ISRXMINI || ISSINP80) {
...
__myevic__ void ResetDataFlash() {
    ...
    //      dfDimTimeout = 20;          //see dfScrMainTime; set in
    //      GetMainScreenDuration()
    //      dfStealthPuffsCnt = 0;
    //      dfDimOffMode = 0;
...
__myevic__ void DFCheckValuesValidity() {
    /*
            ...
            if ( dfTempAlgo != 1 && dfTempAlgo != 2 && dfTempAlgo != 3 &&
    dfTempAlgo != 4 )
                    dfTempAlgo = 1;
            ...
            //{
                    ...
                    for ( i = 0 ; i < PWR_CURVE_PTS ; ++i )
                    {
                            ...
                            if (( dfPwrCurve[i].time > 250 ||
    dfPwrCurve[i].power > 200 )
                            ||	( i == 0 && dfPwrCurve[i].time != 0 )
                            ||	( i != 0 && dfPwrCurve[i].time <=
    dfPwrCurve[i-1].time ))
                            {
                                    ResetPowerCurve();
                                    break;
    ...
    //75 150
            ...
            if ( dfClkRatio < RTC_MIN_CLOCK_RATIO || dfClkRatio >
    RTC_MAX_CLOCK_RATIO )
                    dfClkRatio = RTC_DEF_CLK_RATIO;
            ...
            if (( dfBatteryModel >= GetNBatteries() ) && ( dfBatteryModel !=
    BATTERY_CUSTOM ))
                    dfBatteryModel = 0;
    ...
    //	MemSet( DataFlash.p.UnusedCA, 0, sizeof(DataFlash.p.UnusedCA) );
            ...
            if ( dfMaxBoardTemp > 99 || dfMaxBoardTemp < 20 ) dfMaxBoardTemp =
    70;
...
__myevic__ uint32_t ReadDataFlash(uint32_t u32Addr, uint32_t *pu32Buf) {
    ...
    for (offset = 0; offset < DATAFLASH_PARAMS_SPACE;
         offset += DATAFLASH_PARAMS_SIZE) {
        if (FMC_Read(u32Addr + offset) == ~0 &&
            FMC_Read(u32Addr + offset + 4) == ~0) {
            break;
        }
        offset += DATAFLASH_PARAMS_SIZE;
    }
...
__myevic__ void WriteDataFlash(uint32_t u32Addr, const uint32_t *pu32Data) {
    ...
    if (offset >= DATAFLASH_PARAMS_SPACE) {
        ...
        if (offset % FMC_FLASH_PAGE_SIZE == DATAFLASH_PARAMS_SIZE) {
            FMC_Erase(u32Addr + offset - DATAFLASH_PARAMS_SIZE +
                      FMC_FLASH_PAGE_SIZE);
        }
    }
...
__myevic__ void InitDataFlash() {
    ...
    */
    ...
    if ((dfMagic == DFMagicNumber) &&
        (CalcPageCRC(DataFlash.params) == dfCRC)) {
...
const uint8_t ProfileFilter[32] = {
    /* 0000 */ 0b00000000,
    /* 0008 */ 0b00101111, // 8 9 A B C D E F   see typedef struct dfParams in
                           // dataflash.h
    /* 0010 */ 0b11111111,
    /* 0018 */ 0b11111111,
    /* 0020 */ 0b00000000,
...
__myevic__ void LoadProfile(int p) {
    ...
    if ((params->Magic == DFMagicNumber) &&
        (params->PCRC == CalcPageCRC((uint32_t *)params))) {
        s  = (uint8_t *)params;
        ...
        *(uint32_t *)&d2[offsetof(dfParams_t, Status2)] =
            old_status2 | new_status2;
