...
__myevic__ void DrawPwrLine(int pwr, int line) {
    ...
    {
        // if ( EditModeTimer && ( EditItemIndex == 2 ) && ( !PD2 || !PD3 ) &&
        // !dfStatus.onewatt )
        //{
...
__myevic__ void DrawTempLine(int line) {
    ...
    {
        DrawValueRight(x, y,
                       dfStatus.IsCelsius ? FarenheitToC(AtoTemp) : AtoTemp, 0,
                       fset, 3);
        /*
                        if ( dfStatus.IsCelsius )
                        {
                                DrawValueRight( x, y, FarenheitToC( AtoTemp ),
           0, fset, 3 );
                        }
                        else
                        {
...
__myevic__ void DrawCoilLine(int line) {
    ...
    if ((!lock && Set_NewRez_dfRez) ||
        !AtoRez) // Set_NewRez_dfRez show AtoRez on start
    {
        rez = AtoRez;
    } else {
        rez = dfResistance;
    ...
    if (rez) { // blink if res not for this mode
    ...
    for (int i = 0; i < count; ++i) {
        ...
        case 0: // Current
        {
            if (gFlags.battery_charging) {
                DrawString(gFlags.firing ? String_AMP_s : String_UCH_s, 0,
                           line + 2);
                if (gFlags.firing) {
                ...
                DrawValue(27, line, (gFlags.firing) ? AtoCurrent : 0, 1, 0x1F,
                          3);
            }
        ...
        case 5: {
            ...
            if (a == 5) {
                ...
                // DrawValue( 0, 57, RTCReadRegister( RTCSPARE_VV_BASE ), 0,
                // 0x01, 0 );
                // vv = vv * 86400 / ( t ? : 1 );
        ...
        case 7: // Battery voltage
        {
            ...
            // DrawValue( 27, line, gFlags.firing?RTBattVolts:BatteryVoltage, 2,
            // 0x1F, 3 );
            if (dfAPT == dfAPT3) {
                if (i == 0) {
                    int b = NumBatteries > 1 ? 1 : 0;
                    DrawValue(27, line,
                              gFlags.firing ? RTBVolts[b] : BattVolts[b], 2,
                              0x1F, 3);
                } else {
                    DrawValue(27, line,
                              gFlags.firing ? RTBVolts[0] : BattVolts[0], 2,
                              0x1F, 3);
                }
            } else {
                DrawValue(27, line,
                          gFlags.firing ? RTBattVolts : BatteryVoltage, 2, 0x1F,
                          3);
            }
            DrawImage(57, line + 2, 0x7D);
            break;
        ...
        {
            DrawString(String_BATT_s, 0, line + 2);
            DrawValueRight(55, line + 2,
                           gFlags.firing ? RTBVTotal : BattVoltsTotal, 2, 0x0B,
                           4);
            DrawImage(57, line + 2, 0x7D);
            break;
        }
...
__myevic__ void DrawInfoLines() {
    ...
    /*
       ...
       MilliJoules
                   ...
                   // unsigned int amps;
                   // amps = 1000 * BattVoltsTotal / ( 10 * AtoRez +
       NumBatteries * BatteryIntRez );
                   // //amps = 10 * BattVoltsTotal	/ ( 10 * AtoRez +
       NumBatteries * BatteryIntRez );
    ...
    if (ISMODEVW(dfMode)) {
        ...
        if (dfStatus.preheat) {
            if (!PreheatDelay || gFlags.osc_1hz) {
                if (dfStatus2.smart_ph && NextPreheatTimer &&
...
__myevic__ void ShowMainView() {
    ...
    } else if (dfMode == 6) // smart
    {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            // DrawFillRect( const int x1, const int y1,const  int x2, const int
            // y2, const int color)
            DrawFillRect(x1 + i * 6, 72 - 26 * p / pMax, x1 + x2 + i * 6, 72,
                         1); // 74 = bottom, 30 = height
        ...
        DrawHLineDots(0, 41, 63, 1); // upper line
                                     ...
                                     /*
                                                     ...
                                                     {
                                                             for ( i = 0;
                                                                       dfSavedCfgPwr[(int)ConfigIndex] / (
                                        MaxPower / 15 ) > i;
                                                                       ++i )
                                                             {
                                                                     DrawFillRect( 0, 89 - 5*i, 62, 90 - 5*i,
                                        1 ); //94 95
                                                             }
                                                     ...
                                                     {
                                                             v20 = SMARTPowers[ 2 * v15 + 1 ] / (MaxPower /
                                        15);
                                                             v17 = 94 - 5 * v20; //99
                                                             for ( j = 0; ; ++j )
                                                             {
                                                                     v19 = dfSavedCfgPwr[(int)ConfigIndex] /
                                        (MaxPower / 15);
                                                                     if ( v19 <= j )
                                                                             break;
                                                                     if ( v19 >= v20 && v20 - 1 <= j )
        ...
        /*
           ...
           0x1F, 3 ); //smart power
                                ...
                                else
                                {
                                        ShowBattery();
                                        ShowFireDuration( 0 );
                                        //DrawValue( 10, 110, FireDuration, 1,
           0x29, 2 );
    ...
    if (dfMode != 6) {
        ...
        if (Screen == 2 || EditModeTimer) {
            ...
            } else if (!dfStatus.nologo && dfStatus.logomid &&
                       !HideLogo) // mid logo
            {
                int h = GetLogoHeight();
        ...
        if ((Screen == 1) && !HideLogo && !dfStatus.nologo &&
            !dfStatus.logomid) {
            int h = GetLogoHeight();
            int y = 0;
            int y2;
            ...
            if (h) {
                if (h > 40) {
                    if (!(dfStatus2.anim3d || dfStatus.clock)) {
                        if (dfUIVersion == 0) {
                            y2 = 60;
                            // DrawFillRect( 0, 45, 63, 60, 0 );   //erase 1-st
                            // info line
                            // DrawFillRect( 0, 0, 63, 60, 0 ); // erase with
                            // upper line
                            y = 10;
                        } else {
                            y2 = 66;
...
__myevic__ void DrawClock(int line) {
    ...
    DrawLine(32, c, 32 + ((sine(h) * 15) >> 16), c - ((cosine(h) * 15) >> 16),
             1, 3);
    DrawLine(32, c, 32 + ((sine(m) * 21) >> 16), c - ((cosine(m) * 21) >> 16),
             1, 2);
    DrawLine(32, c, 32 + ((sine(s) * 23) >> 16), c - ((cosine(s) * 23) >> 16),
             1, 1);
}
...
__myevic__ void DrawDigitClock(int line, int infoline) {
    ...
    if (!dfStatus.timebig || infoline) {             // small
