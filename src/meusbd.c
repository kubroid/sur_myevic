...
const uint8_t usbdDevice[] = {
    ...
    /* idProduct */
    USBD_PID & 0x00FF, (USBD_PID & 0xFF00) >> 8, 0x00, 0x00, /* bcdDevice */
    0x01,                                                    /* iManufacture */
    0x02,                                                    /* iProduct */
    0x03,                                                    /* iSerialNumber */
...
const uint8_t usbdConfigDesc[] = {
    ...
    /* wTotalLength */
    DESC_LEN_WITHOUT_VCOM & 0x00FF, (DESC_LEN_WITHOUT_VCOM & 0xFF00) >> 8,
    0x01, /* bNumInterfaces */
    ...
    sizeof(usbdHIDReport) & 0x00FF, (sizeof(usbdHIDReport) & 0xFF00) >> 8,
...
const uint8_t gu8StringLang[4] = {4,           /* bLength */
                                  DESC_STRING, /* bDescriptorType */
                                  0x09, 0x04};

/*!<USB Vendor String Descriptor */
const uint8_t gu8VendorStringDesc[] = {
    16, DESC_STRING, 'N', 0, 'u', 0, 'v', 0, 'o', 0, 't', 0, 'o', 0, 'n', 0};

/*!<USB Product String Descriptor */
const uint8_t gu8ProductStringDesc[] = {26,          /* bLength          */
                                        DESC_STRING, /* bDescriptorType  */
                                        'H',         0, 'I', 0, 'D', 0, ' ', 0,
                                        'T',         0, 'r', 0, 'a', 0, 'n', 0,
                                        's',         0, 'f', 0, 'e', 0, 'r', 0};
...
const uint8_t gu8StringSerial[26] = {26,          // bLength
                                     DESC_STRING, // bDescriptorType
                                     'A',         0, '0', 0, '2', 0, '0', 0,
                                     '1',         0, '5', 0, '0', 0, '8', 0,
                                     '1',         0, '3', 0, '0', 0, '2', 0};

const uint8_t *usbdStrings[] = {gu8StringLang, gu8VendorStringDesc,
                                gu8ProductStringDesc, gu8StringSerial};

const uint8_t *usbdHIDReportDesc[] = {usbdHIDReport, 0};

const uint32_t usbdHIDReportSize[] = {sizeof(usbdHIDReport), 0};

const uint32_t usbdConfigHidDescIdx[] = {(LEN_CONFIG + LEN_INTERFACE), 0};

const S_USBD_INFO_T usbdDescriptors = {usbdDevice,        usbdConfigDesc,
                                       usbdStrings,       usbdHIDReportDesc,
                                       usbdHIDReportSize, usbdConfigHidDescIdx};

const S_USBD_INFO_T usbdVCOMDescriptors = {
    usbdDevice,        usbdVCOMConfigDesc, usbdStrings,
    usbdHIDReportDesc, usbdHIDReportSize,  usbdConfigHidDescIdx};
...
__myevic__ void hidSetInReport() {
    ...
    /*
                    case HID_CMD_FMCREAD:
                    {
                            if ( !hidDataIndex )
                            {
                                    uint32_t u32Addr = hidCmd.u32Arg1 +
       hidCmd.u32Signature;
                                    int32_t  u32Size = hidCmd.u32Arg2 -
       hidCmd.u32Signature;
                                    if ( u32Size > 0 )
                                    {
                                            if ( u32Size > FMC_FLASH_PAGE_SIZE )
       ...
       4 )
                                            {
                                                    uint32_t data = FMC_Read(
       u32Addr + i );
                                                    MemCpy( &hidData[
       FMC_FLASH_PAGE_SIZE - u32Size + i ], &data, 4 );
                                            }
                            ...
                            {
                                    USBD_MemCopy(
                                            (uint8_t *)(USBD_BUF_BASE +
       USBD_GET_EP_BUF_ADDR(EP2)),
                                            &hidData[ FMC_FLASH_PAGE_SIZE -
       hidDataIndex ],
                                            EP2_MAX_PKT_SIZE
                                    );
                                    USBD_SET_PAYLOAD_LEN( EP2, EP2_MAX_PKT_SIZE
...
//-------------------------------------------------------------------------
__myevic__ void usbdEP2Handler() { hidSetInReport(); }
...
__myevic__ uint32_t hidGetProfile( CMD_T *pCmd )
{
        ...
        else
        {
                ...
                p =
(dfParams_t*)(DATAFLASH_PROFILES_BASE+DATAFLASH_PARAMS_SIZE*(u32ProfileNum-1));
        }

        MemCpy( &hidData[0], (uint8_t *)p, DATAFLASH_PARAMS_SIZE );
        MemCpy( &hidData[DATAFLASH_PARAMS_SIZE], ((uint8_t *)&DataFlash.i),
DATAFLASH_INFOS_SIZE );
...
__myevic__ uint32_t hidGetMonData(CMD_T *pCmd) {
    ...
    //	myprintf( "Get Monitoring Data command - Start Addr: %d    Param Len:
    //%d\n", pCmd->u32Arg1, pCmd->u32Arg2 );
    ...
    for (int i = 0; i < NumBatteries; ++i) {
        mondata->BatteryVoltage[i] =
            (gFlags.firing ? RTBVolts[i] : BattVolts[i]) - 275;
    }
...
__myevic__ void hidGetOutReport(uint8_t *pu8Buffer, uint32_t u32BufferLen) {
    ...
    /*
                    case HID_CMD_SETPROFILE:
                    {
                            USBD_MemCopy( hidDataPtr, pu8Buffer,
       EP3_MAX_PKT_SIZE );
                            ...
                            if ( hidDataIndex >= u32DataSize )
                            {
                                    ...
                                    dfInfos_t  *i =
       (dfInfos_t*)&hidData[DATAFLASH_PARAMS_SIZE];

                                    if ( i->Format != HID_PROFILE_FORMAT ||
       p->Magic != DFMagicNumber )
                                    {
                                            //myprintf( "Incompatible parameters
       format.\n" );
                                            break;
                                    }
                                    ...
                                    if ( u32StartAddr == 0 ) // profile #
                                    {
                                            ...
                                            MemCpy( DataFlash.params, p,
       DATAFLASH_PARAMS_SIZE );
                                    ...
                                    else
                                    {
                                            ...
                                            uint8_t page[FMC_FLASH_PAGE_SIZE]
       __attribute__((aligned(4)));

                                            uint32_t offset = p->Profile *
       DATAFLASH_PARAMS_SIZE;

                                            MemCpy( page,
       (void*)DATAFLASH_PROFILES_BASE, FMC_FLASH_PAGE_SIZE );
                                            MemCpy( page + offset, p,
       DATAFLASH_PARAMS_SIZE );
                                            ...
                                            FMCEraseWritePage(
       DATAFLASH_PROFILES_BASE, (uint32_t*)page );
    ...
    */
    ...
    case HID_CMD_SETPARAMS: {
        ...
        if (hidDataIndex >= u32DataSize) {
            ...
            if (Checksum((uint8_t *)df->params, FMC_FLASH_PAGE_SIZE - 4) ==
                df->Checksum) {
                // myprintf( "\tCompany ID ............................
                // [0x%08x]\n",
                //			df->i.fmcCID );
                ...
                /*
                                                        ...
                                                        {
                                                                //myprintf(
                   "Incompatible parameters format.\n" );
                   ...
                   Custom Battery
                                                                        o =
                   offsetof( dfStruct_t, p ) + 0xCA;
                                                                        MemCpy(
                   &CustomBattery.V2P, (uint8_t*)(&hidDFData) + o, 48 );
    ...
    case HID_CMD_SETLOGO: {
        ...
        if (hidDataIndex < FMC_FLASH_PAGE_SIZE &&
            hidDataIndex + u32ByteCount < u32DataSize) {
            break;
        }
    ...
    /*
       ...
       hidDataIndex + u32ByteCount < u32DataSize )
                            ...
                            if ( veo )
                            {
                                    //myprintf( "Data Flash Verify error!
       0x%x\n", 4 * veo - 4 );
                            }
                            ...
                            //myprintf( "g_u32BytesInPageBuf %d, u32LenCnt
