...
const uint8_t usbdVCOMConfigDesc[] = {
    ...
    /* wTotalLength */
    DESC_LEN_WITH_VCOM & 0x00FF, (DESC_LEN_WITH_VCOM & 0xFF00) >> 8,
    0x03, /* bNumInterfaces */
    ...
    0x10, 0x01, /* Communication device compliant to the
                   communication spec. ver. 1.10 */
    ...
    0x00, /* BIT0: Whether device handle call management itself. */
          /* BIT1: Whether device can send/receive call management
             information over a Data Class Interface 0 */
    0x02, /* Interface number of data class interface optionally
    ...
    (EP4_MAX_PKT_SIZE & 0xFF00) >> 8, 0x01, /* bInterval        */
    ...
    (EP5_MAX_PKT_SIZE & 0xFF00) >> 8, 0x00, /* bInterval        */
    ...
    (EP6_MAX_PKT_SIZE & 0xFF00) >> 8, 0x00, /* bInterval        */
};
...
__myevic__ void VCOM_Poll() {
    ...
    if (VCOM_TxReady) {
        ...
        if (comRbytes) {
            ...
            /* Prepare a zero packet if previous packet size is EP2_MAX_PKT_SIZE
               and
               no more data to send at this moment to note Host the transfer has
               been done */
...
__myevic__ void VCOM_EP6Handler() {
    ...
    //	if ( i32Len )
    //	{
    //		USBD_MemCopy(	(uint8_t *)gRxBuf, (uint8_t*)(USBD_BUF_BASE +
    // USBD_GET_EP_BUF_ADDR(EP6)),
    //						i32Len );
    //	}
...
__myevic__ char VCOM_Putc(char c, FILE *out) {
    if (!USBD_IS_ATTACHED() || !(gCtrlSignal & VCOM_LINESTATE_MASK_DTR)) {
...
__myevic__ void VCOM_ClassRequest(uint8_t *token) {
    if (token[0] & 0x80) /* request data transfer direction */
    {
        ...
        case CDC_SET_CONTROL_LINE_STATE: {
            if (token[4] == VCOM_INTERFACE) { /* VCOM-1 */
                gCtrlSignal = token[3];
