...
static const char mon_days[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
...
__myevic__ time_t time_to_epoch(const struct tm *ltm, int utcdiff) {
    ...
    return (tdays * 86400) + (utc_hrs * 3600) + (ltm->tm_min * 60) +
           ltm->tm_sec;
}
...
__myevic__ struct tm *epoch_to_time(struct tm *            timep,
                                    register const time_t *timer) {
    time_t                 time = *timer;
...
__myevic__ void RTCStart(S_RTC_TIME_DATA_T *d) {
    ...
    // RTC_SetAlarmTime(uint32_t u32Hour, uint32_t u32Minute, uint32_t
    // u32Second, uint32_t u32TimeMode, uint32_t u32AmPm);
...
__myevic__ void RTCAdjustClock(int seconds) {
    ...
    if (seconds) {
        ...
        if (gFlags.has_x32) {
            ...
            uint64_t f1 = ((((RTC->FREQADJ & RTC_FREQADJ_INTEGER_Msk) >>
