...
//=========================================================================
//----- (000017B0) --------------------------------------------------------
__myevic__ void SSD1327_SetColsRowsAddrs(char col_start, char col_end,
                                         char row_start, char row_end) {
...
const uint8_t SSD1327_InitSeq[] = {0xAE, 0xA0, 0x40, 0xA1, 0x00, 0xA2, 0x00,
                                   0xA4, 0xA8, 0x7F, 0xAB, 0x01, 0x81, 0xA6,
                                   0xB1, 0x31, 0xB3, 0xB1, 0xB4, 0xB5, 0xB6,
                                   0x0D, 0xBC, 0x07, 0xBE, 0x07, 0xD5, 0x02};
...
__myevic__ uint32_t SSD1327_Image(int x, int y, uint8_t img, int color) {
    if (img == 0x88 || img == 0x8B || img == 0x91 || img == 0x92 ||
        img == 0x9A) {
        y += 2;
    }
...
//-------------------------------------------------------------------------
__myevic__ uint32_t SSD1327_Bitmap(int x, int y, const image_t *image,
                                   int color) {
    int     sblinebase, sbi;
    uint8_t sbbyte;
...
//=========================================================================
//----- (00006444) --------------------------------------------------------
__myevic__ void SSD1327_WriteBytes(const int isData, const uint8_t data[],
                                   const int len) {
