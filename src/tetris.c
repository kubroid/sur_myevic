...
void ttSetTimeoutDelay(int v) { ttTimeoutsTable[ttCurrentTimeout].delay = v; }

void ttCallTimeouts() {
    ttTimeoutMask = 1;

    for (ttCurrentTimeout = 0; ttCurrentTimeout < TT_NUM_TIMERS;
         ++ttCurrentTimeout) {
...
void ttCLSBuf() { MemClear2(0, ScreenBuffer, SCREEN_BUFFER_SIZE); }
...
void CompletedLines() {
    ...
    if (currentPiece.Row > 5)
        bottomRow = currentPiece.Row - 4;
    else
        bottomRow = 1;

    if (currentPiece.Row == 22)
        topRow = 21;
    else
        topRow = currentPiece.Row;
            ...
            for (int currentRow = 2; currentRow < 22; currentRow++) {
                ...
                if (noLine) {
                    ...
                    for (int r = currentRow + 1; r < 22; r++) {
                        for (int c = 2; c < 12; c++) {
                            if (tetrisScreen[c][r])
                                tetrisScreen[c][r - 1] = 2;
                            else
                                tetrisScreen[c][r - 1] = 3;
                        }
                    }
                }
            }
...
void DrawLandedPiece() {
    ...
    if (CurPieceNumber)
        FastMove += noRotate * 10; // score bonus
    FastMove += noShift * 20;
    noRotate = 1;
    noShift  = 1;
...
void ttStartScreen() {
    ...
    if (!PE0 && (PD2 && PD3)) {
        ...
        switch (dfTTSpeed) {
        case 0:
            DrawStringCentered(String_Easy, 95);
            break;
        case 1:
            DrawStringCentered(String_Normal, 95);
            break;
        case 2:
            DrawStringCentered(String_Survival, 95);
            break;
        }
