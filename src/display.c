...
const uint8_t ByteMaskRight[] = {0x00, 0x01, 0x03, 0x07,
                                 0x0F, 0x1F, 0x3F, 0x7F};
...
__myevic__ void DrawTime(int x, int y, S_RTC_TIME_DATA_T *rtd, int colors) {
    // 24h only
    if (gFlags.draw_edited_item)
        colors = 0x1F;

    if (colors & 0x10)
        DrawValue(x, y, rtd->u32Hour, 0, 0x1F, 2);
    if (colors & 0x08)
        DrawImage(x + 17, y, 0xDD);
    if (colors & 0x04)
        DrawValue(x + 21, y, rtd->u32Minute, 0, 0x1F, 2);
    if (colors & 0x02)
        DrawImage(x + 38, y, 0xDD);
    if (colors & 0x01)
        DrawValue(x + 42, y, rtd->u32Second, 0, 0x1F, 2);
}
...
__myevic__ void DrawDate(int x, int y, S_RTC_TIME_DATA_T *rtd, int colors) {
    ...
    const datefmt_t format[] = {{0xC1, 0, 16, 32, 13, 29},
                                {0xD6, 16, 0, 32, 12, 28},
                                {0xD6, 0, 16, 32, 12, 28},
                                {0xD9, 44, 28, 0, 24, 40}};
    ...
    if (colors & 0x10)
        DrawValue(x + f->dayoffset, y, rtd->u32Day, 0, 0x0B, 2);
    if (colors & 0x04)
        DrawValue(x + f->monthoffset, y, rtd->u32Month, 0, 0x0B, 2);
    if (colors & 0x01)
        DrawValue(x + f->yearoffset, y, rtd->u32Year, 0, 0x0B, 4);
    if (colors & 0x08)
        DrawImage(x + f->sep1offset, y, f->separator);
    if (colors & 0x02)
        DrawImage(x + f->sep2offset, y, f->separator);
    ...
    const uint8_t *WEEKS[] = {String_Sunday,    String_Monday,   String_Tuesday,
                              ...
                              String_Saturday};
    DrawStringCentered(
        WEEKS[dayofweek(rtd->u32Day, rtd->u32Month, rtd->u32Year)], y);
}
...
//=========================================================================
//----- (000051F8) --------------------------------------------------------
__myevic__ void DrawHLine(const int x1, const int y, const int x2,
                          const int color) {
    int inc = (x1 < x2) * 2 - 1;
...
__myevic__ void DrawHLineDots(const int x1, const int y, const int x2,
                              const int color) {
    int inc = (x1 < x2) * 2 - 1;
...
//=========================================================================
__myevic__ void DrawVLine(const int x, const int y1, const int y2,
                          const int color) {
    int inc = (y1 < y2) * 2 - 1;
...
//=========================================================================
//----- (0000575C) --------------------------------------------------------
__myevic__ void DrawFillRect(const int x1, const int y1, const int x2,
                             const int y2, const int color) {
    for (int y = y1; y <= y2; ++y) {
        DrawHLine(x1, y, x2, color);
    }
}
__myevic__ void DrawFillRectLines(const int x1, const int y1, const int x2,
                                  const int y2, const int color) {
    for (int y = y1; y <= y2; ++y) {
...
__myevic__ uint32_t DrawImageRight(const int x, const int y,
                                   const uint8_t img) {
    switch (DisplayModel) {
    case 0:
...
/*
__myevic__ void DrawImgImgImg( const int x, const int y, const uint8_t img1,
const uint8_t img2, const uint8_t img3 )
{
        switch ( DisplayModel )
        {
                case 0:
...
//=========================================================================
//----- (000058A4) --------------------------------------------------------
__myevic__ void DrawValue(int x, int y, int v, uint8_t dp, uint8_t z,
                          uint8_t nd) {
    uint8_t str[12];
...
//-------------------------------------------------------------------------
__myevic__ void DrawValueRight(int x, int y, int v, uint8_t dp, uint8_t z,
                               uint8_t nd) {
    uint8_t str[12];
...
//=========================================================================
//----- (000058A4) --------------------------------------------------------
__myevic__ void DrawValueInv(int x, int y, int v, uint8_t dp, uint8_t z,
                             uint8_t nd) {
    uint8_t str[12];
...
__myevic__ void DrawPixel(int x, int y, int color) { DrawPoint(x, y, color); }