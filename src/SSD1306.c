...
//=========================================================================
//----- (00005628) --------------------------------------------------------
__myevic__ uint32_t SSD1306_Bitmap(int x, int y, const image_t *image,
                                   int color) {
    uint32_t shift;
    uint32_t h, w;
    uint32_t bm_ptr;
        ...
        for (w = 0; w < image->width; ++w) {
            ...
            if (shift) {
                if (addr < SCREEN_BUFFER_SIZE) {
                    ...
                    ScreenBuffer[addr] |=
                        (pixels << shift) & ByteMaskLeft[shift];
                }
                if (addr + 0x40 < SCREEN_BUFFER_SIZE) {
                    ScreenBuffer[addr + 0x40] &= ByteMaskLeft[shift];
                    ScreenBuffer[addr + 0x40] |=
                        (pixels >> (8 - shift)) & ByteMaskRight[shift];
                }
            } else {
                if (addr < SCREEN_BUFFER_SIZE) {
...
//=========================================================================
//----- (000064C8) --------------------------------------------------------
__myevic__ void SSD1306_WriteBytes(const int isData, const uint8_t data[],
                                   const int len) {
