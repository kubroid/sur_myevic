...
__myevic__ void KeyRepeat() {
    ...
    if (PD2 == PD3) {
        if (KeyUpTimer) {
            if (!--KeyUpTimer && Screen == 1 &&
                (dfTCPower >= 1000 || dfMode == 6))
                MainView();
        }
        KeyTicks = 0;
        KRDelay  = 6;
...
__myevic__ void SetClicksAction(uint8_t num) {
    // multi fire clicks
    switch (num)
    // switch ( gFlags.asleep ? dfClick[FireClickCount-3] :
    // dfClick[FireClickCount-2] )
    {
    default:
...
__myevic__ void GetUserInput() {
    ...
    if ((!PE0 || AutoPuffTimer) && PD2 && PD3) {
        ...
        if (!PE0) // AutoPuffTimer = 0; //fb pressed
        {
            if (dfStatus.autopuff && dfAutoPuffTimer) {
                ...
                {
                    // AutoPuffTimer = (uint16_t)dfAutoPuffTimer * 10 -
                    // FireDuration * 10;
                    AutoPuffTimer =
                        ((uint16_t)dfAutoPuffTimer - FireDuration) * 10;
                    gFlags.apuff = 1;
                }
            }
        }
        ...
        if (gFlags.firing) {
            if (LastInputs == 1) {
                ...
                StopFire(); // 2
                // gFlags.refresh_display = 1; //bad idea // for correct last
                // FireDuration in TC
            }
            gFlags.user_idle = 1;
            LastInputs       = -1;
        ...
        if (gFlags.usb_attached) {
            if (ISVTCDUAL) {
                if (NumBatteries == 1) {
                    ...
                    } else if (PD1 && gFlags.battery_charging &&
                               BatteryVoltage >= 414) {
                        UserInputs     = 13;
                        BattProbeCount = 1;
                    }
    ...
    if (KeyPressTime == 6) {
        ...
        if (UserInputs == 4 || UserInputs == 5 || UserInputs == 6 ||
            UserInputs == 7)
            return;
        ...
        if (UserInputs == 1) {
            ...
            case 1:
                ...
                if (Screen != 1 || !EditModeTimer ||
                    (EditItemIndex != 4 && EditItemIndex != 5)) {
                    Event = 1; // fire
                }
            ...
            default:
                ...
                // case 7:
                //	FireClicksEvent = 31;	// board temp screen from on
                // state
                //	break;
        ...
        } else if (UserInputs == 2) // right button
        {
            ...
            } else if (dfStatus.keylock && !EditModeTimer && Screen != 51 &&
                       !IsMenuScreen()) {
        ...
        } else if (UserInputs == 7) // all 3 buttons
        {
            if (!dfStatus.off &&
                (Screen == 1 ||
                 Screen == 0)) //( !dfStatus.off && !IsMenuScreen() )
            {
        ...
        {
            if (!EditModeTimer &&
                !IsMenuScreen()) //! IsMenuScreen() for no key lock in menu
            {
                if (dfStatus.off) {
        ...
        if (UserInputs == 1) {
            if ((Screen == 1) && (EditModeTimer > 0)) {
                uint8_t a;
                if (EditItemIndex == 4 ||
                    EditItemIndex == 5) // 4 = 3-d info line
                {
                    if (EditItemIndex == 4)
                        a = dfAPT3;
                    else
    ...
    */
        ...
        if (UserInputs == 1) {
            ...
            {
                KeyPressTime =
                    pt; // dfProtec * 10 + 50; //FIRE_PROTEC_MAX * 10 + 100;
...
__myevic__ int EvtPlusButton() {
    ...
    case 106: {
        ...
        case 8:
        case 14:
            if (SetTimeRTD.u32Year < RTC_YEAR2000 + 1000)
                ++SetTimeRTD.u32Year;
            break;
        case 1:
        case 6:
    ...
    case EVENT_SET_JOULES: // scr = event
    {
        if (++dfVVRatio > VVEL_MAX_RATIO) {
            if (KeyTicks < 5)
                dfVVRatio = VVEL_MIN_RATIO;
            else
                dfVVRatio = VVEL_MAX_RATIO;
        }
        UpdateDFTimer = 50;
        // EditModeTimer = 3000;
        ScreenDuration         = 60;
...
__myevic__ int EvtMinusButton() {
    ...
    case 106: {
        ...
        case 8:
        case 14:
            if (SetTimeRTD.u32Year > RTC_YEAR2000)
                --SetTimeRTD.u32Year;
            break;
        case 1:
        case 6:
