...
__myevic__ void InitHardware() {
    ...
    if (dfStatus.x32off) {
        ...
        SYS->GPF_MFPL |=
            (SYS_GPF_MFPL_PF0MFP_X32_OUT | SYS_GPF_MFPL_PF1MFP_X32_IN);
...
__myevic__ void InitRTC() {
    ...
    //	}
    if (!MilliJoules)
        MilliJoules = dfJoules;
    if (!MilliJoulesDay)
        MilliJoulesDay = dfJoulesDay;
...
__myevic__ void DevicesOnOff(int off) {
    if (off) {
        ...
        if (ISVTCDUAL || ISCUBOID || ISCUBO200 || ISRX200S || ISRX23 ||
            ISRX300 || ISPRIMO1 || ISPRIMO2 || ISPREDATOR || ISGEN3 ||
            ISINVOKE || ISRX2 || ISSINFJ200 || ISRX217) {
        ...
        if (!ISRX2 && !ISSINP80 && !ISINVOKE && !ISRX217) {
            ...
            PD0 = 0; // 400048C0
            GPIO_SetMode(PD, GPIO_PIN_PIN0_Msk,
                         GPIO_MODE_OUTPUT); // 400040C0 1 1
        }
        ...
        SYS->GPE_MFPH |=
            (SYS_GPE_MFPH_PE11MFP_GPIO | SYS_GPE_MFPH_PE12MFP_GPIO |
        ...
        USBD_CLR_INT_FLAG(USBD_INTSTS_WAKEUP | USBD_INTSTS_FLDET |
                          USBD_INTSTS_BUS | USBD_INTSTS_USB);
    ...
    } else // Device On
    {
        ...
        SYS->GPE_MFPH &= ~(SYS_GPE_MFPH_PE11MFP_Msk | SYS_GPE_MFPH_PE12MFP_Msk |
                           SYS_GPE_MFPH_PE13MFP_Msk);
        SYS->GPE_MFPH |=
            (SYS_GPE_MFPH_PE11MFP_SPI0_MOSI0 | SYS_GPE_MFPH_PE12MFP_SPI0_SS |
        ...
        if (ISVTCDUAL) {
            ...
            GPIO_SetMode(PD, GPIO_PIN_PIN7_Msk,
                         GPIO_MODE_INPUT);          // 40C0 0x80 0     ISSINP80
        ...
        if (ISVTCDUAL || ISCUBOID || ISCUBO200 || ISRX200S || ISRX23 ||
            ISRX300 || ISPRIMO1 || ISPRIMO2 || ISPREDATOR || ISGEN3 || ISRX2 ||
            ISINVOKE || ISSINFJ200 || ISRX217) {
            SetADCState(3, 1);
            SetADCState(13, 1);
...
__myevic__ void SleepIfIdle() {
    if (!gFlags.firing && !NoEventTimer) {
        if ((Screen == 0) && (SleepTimer == 0) && (gFlags.user_idle)) {
            ...
            if (dfDimOffMode == 1 || ((BatteryVoltage <= BatteryCutOff + 20) &&
                                      !gFlags.usb_attached)) {
                dfStatus.off = 1;
                // Screen = 0;
                // LEDOff(); ?
...
__myevic__ void Monitor()
{
        if ( gFlags.firing )
        {
                ...
                if ( ISMODETC(dfMode) )
                {
                        myprintf( " SPWR=%d RPWR=%d CELS=%d STEMP=%d
RTEMP=%d\n",
                                                dfTCPower,
                                                AtoPower( AtoVolts ),
                                                dfIsCelsius ? 1 : 0,
                                                dfTemp,
                                                dfIsCelsius ? FarenheitToC(
AtoTemp ) : AtoTemp
                                        );
                }
        ...
        else
        {
                myprintf( "STANDBY "
                                        ...
                                        dfIsCelsius ? BoardTemp : CelsiusToF(
BoardTemp ),
                                        AtoStatus,
                                        AtoRez,
                                        AtoRezMilli,
...
__myevic__ void Main() {
    ...
    while (1) {
        ...
        if (gFlags.tick_100hz) {
            ...
            if (gFlags.firing &&
                (BoardTemp > dfMaxBoardTemp || (ISSINFJ200 && AkkuTemp > 70))) {
                Overtemp();
            }
            ...
            if ((dfStatus2.anim3d) && (Screen == 1) && (dfMode != 6) &&
                (!EditModeTimer) && !HideLogo && !SplashTimer) {
                anim3d(0); // logo
            }
        ...
        if (gFlags.tick_10hz) {
            ...
            if (!gFlags.MainContrast && Screen != 60 && Screen != 5 &&
                Screen != 0 && !gFlags.firing) //&& Screen != 2)
            {
            ...
            if (dfStatus.invert) {
                ...
                int st = (dfStealthOn == 1) ||
                         !gFlags.screen_on; //|| ScreenDuration == 0 ;

                // myprintf( "st=%d dfSt=%d scrOn=%d Scr=%d inv=%d dur=%d\n",
                //			st, dfStealthOn, gFlags.screen_on,
                // Screen,
                // gFlags.inverse, ScreenDuration );

                if ((Screen == 0 || Screen == 5) ||
                    (st && !ScreenDuration) // || ( Screen == 5 && st &&
                                            // !ScreenDuration )
                    || (Screen == 2 && st)
            ...
            if (gFlags.firing) {
                if (dfStatus.fireflip && gFlags.FireNotFlipped &&
                    FireDuration > 2) {
                ...
                if (!(gFlags.apuff && dfStatus.endlessfire &&
                      FireDuration >= dfProtec)) {
                    ++FireDuration;
                }

                if (dfStatus.pcurve)
                    ++CurveRepeatTimerDuration;

                // if ( gFlags.monitoring )
                //{
                //	Monitor();
                //}
            } else if (dfStatus.fireflip && !gFlags.FireNotFlipped &&
                       Screen == 1) {
                gFlags.FireNotFlipped = 1;
                dfStatus.flipped ^= 1;
                InitDisplay();
            ...
            if (!(gFlags.firing &&
                  ISMODETC(dfMode))) //!(A && B) = !A || !B  DeMorgans Law
            {
                DrawScreen();
            }
        ...
        if (gFlags.tick_5hz) {
            ...
            if (gFlags.firing) {
                ...
                if (!dfStatus.off && Event == 0 && (AtoProbeCount < 12) &&
                    (Screen == 0 || Screen == 1 || Screen == 2 || Screen == 5 ||
                     Screen == 60)) {
                    ProbeAtomizer();
                }
            }
        ...
        if (gFlags.tick_2hz) {
            ...
            gFlags.osc_1hz ^= 1;
            //
            if (!gFlags.battery_charging && !gFlags.nbcr && dfStatus.nbrc &&
                gFlags.rtcinit && (BatteryVoltage > 250) &&
                (BatteryVoltage > dfBattVolt + 20)) {
            ...
            if (gFlags.firing) {
                ...
                // if ( AtoError == 2 )
                if ((AtoStatus == 1 || AtoStatus == 5 ||
                     AtoStatus == 2) // short overcurrent low
                    && !dfStatus.off && Event == 0 && AtoProbeCount >= 12) {
                    AtoStatus = 4;
                    ProbeAtomizer();
                    // ReadAtomizer();
                }
        ...
        if (gFlags.tick_1hz) {
            ...
            if (!gFlags.firing)
                AtoProbeCount = 10; // for quick res mesure in idle (lower -
                                    // better but with fire multiclicks damage)
