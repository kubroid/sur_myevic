...
__myevic__ int GetNBatteries() { return NBATTERIES; }
...
__myevic__ const uint8_t *GetBatteryName() { return Battery->name; }
...
__myevic__ void SetBatMaxPower() {
    // 100th of an Amp
    int Imax = 1000 * (BatteryVoltage - (BatteryCutOff + 10)) / BatteryIntRez;
    ...
    int Pmax =
        90 * (BatteryVoltage - BatteryIntRez * Imax / 1000) * Imax / 1000;
...
__myevic__ int BatteryVoltsToPercent(int bv) {
    ...
    bpc = (v2p[i - 1].percent) +
          (bv - v2p[i - 1].voltage) * (v2p[i].percent - v2p[i - 1].percent) /
              (v2p[i].voltage - v2p[i - 1].voltage);
...
__myevic__ void NewBatteryVoltage() {
    ...
    if (((SavedBatPercent != BatteryPercent) && (++BatPCCmpCnt > 2)) // 2
        ...
        if (BatteryVoltage > 280 &&
            ((++BatSmplCnt > 2 && dfBattVolt > BatteryVoltage) ||
             !dfStatus.nbrc))
        // if ( BatteryVoltage > 280 && ( ( dfBattVolt > BatteryVoltage ) ||
        // !dfStatus.nbrc ) )
        {
            dfBattVolt = BatteryVoltage;
            BatSmplCnt = 0;
...
__myevic__ void ReadBatteryVoltage() {
    ...
    if (!gFlags.firing) {
        ...
        if ((NoEventTimer <= 100) && ((SavedBatVoltage > BatteryVoltage &&
                                       SavedBatVoltage - BatteryVoltage > 1) ||
                                      (BatteryVoltage > SavedBatVoltage &&
                                       BatteryVoltage - SavedBatVoltage > 1))) {
...
__myevic__ int CheckBattery() {
    ...
    do {
        ...
        //               myprintf( "i=%d RTBVolts[0]=%d RTBVolts[1]=%d bv=%d
        //               bc=%d\n", i,
    ...
    if (LowBatVolts && pwr > BatteryMaxPwr
        //&& PowerScale == 100
        && bv >= limit_voltage &&
...
__myevic__ void BatteryChargeDual() {
    ...
    if ((NumBatteries == 0) ||
        ((NumBatteries == 2) && (BatteryVoltage < 250u))) {
        BatteryStatus = 2;
        BatteryPinsSetDual();
        // PA3 = 0;
        // PA2 = 0;
            ...
            if (NumBatteries == 2) {
                ...
                if (ChargeStatus != 5 && ChargeStatus != 6) {
                    if (gFlags.battery_charging) {
                        if (BatteryVoltage < 290) {
                            ...
                            if (ChargeStatus != 4) {
                                if (USBMaxLoad) {
                                    if (USBVolts > 420) {
                                        ChargerTarget =
                                            GetMaxCharge(1000); // 1000;
                                    } else {
                                        ChargerTarget =
                                            GetMaxCharge(500); // 500;
                                        USBMaxLoad = 0;
                                    }
                                }
                                ChargeStatus = 3;
                        ...
                        if (ChargeCurrent > ChargerTarget + 10 ||
                            USBVolts < 400) {
                            if (ChargerDuty) {
                                --ChargerDuty;
                            }
...
__myevic__ void BatteryCharge() {
    ...
    // myprintf( "BBBits=%d BV=%d, BVH=%d, ChMode=%d, ChStat=%d, BS=%d PD1=%d
    // CD=%d EOC=%d\n",
    ...
    } else if ((ISRX23 || ISGEN3) && BattVolts[0] >= 250 &&
               BattVolts[1] >= 250 && BattVolts[2] < 20) {
        NumBatteries = 2;
        NewBatteryData();
    ...
    if (gFlags.usb_attached) {
        if (dfStatus.usbchgoff || dfStatus.usbchghotoff) {
            ...
            if (ChargeStatus != 5) {
                if ((BattVoltsHighest < 420) ||
                    (BBBits && BatteryVoltage < 415)) {
                    Event        = 12;
                    ChargeStatus = 0;
                }
            }
            ...
            if ((ChargeStatus == 4) && (BattVoltsHighest > 422)) {
                ...
                if (EOCTempo >= 10) {
                    if (BBBits && (BattVoltsHighest > BatteryVoltage + 5) &&
                        (ChargeStep < 2)) {
                        ++ChargeStep;
                        ChargeMode   = 2;
                        ChargeStatus = 1;
            ...
            if (ChargeMode != 2) {
                ...
                } else if (BattVoltsHighest < 420)     // max current here //416
                {
                    ...
                    if (BattVoltsHighest < 421)            // 416
                        ...
                        ChargerTarget =
                            GetMaxCharge(300); // 300 - 600 while around 421
                }
                ...
                if ((ChargeCurrent > ChargerTarget + 10) || (USBVolts < 400)) {
                    ...
                    if ((++ChargerTempo > 10 &&
                         ChargeCurrent < tmpChargeCurrent) ||
                        (ChargerTempo > 100 &&
                         ChargeCurrent >= tmpChargeCurrent)) {
                        ChargerTempo = 0;
...
__myevic__ void SaveCustomBattery(const Battery_t *b) {
    ...
    // dfBattery.maxamp = BatteryMaxAmp / 100; //( dfBattMaxAmps? :
    // Battery->maxamp )
...
__myevic__ int CheckCustomBattery() {
    ...
    if (CustomBattery.V2P[0].percent != 0)
        return 0;
    if (CustomBattery.V2P[10].percent != 100)
        return 0;
    ...
    for (i = 0; i < 11; ++i) {
        ...
        if (CustomBattery.V2P[i].voltage < 270 ||
            CustomBattery.V2P[i].voltage > 423)
            return 0;

        if ((i > 0) &&
            (CustomBattery.V2P[i].percent <= CustomBattery.V2P[i - 1].percent))
            return 0;
    }

    if (CustomBattery.cutoff < 270 || CustomBattery.cutoff > 350)
        return 0;
